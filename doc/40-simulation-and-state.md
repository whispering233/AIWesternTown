...
第一版把“玩家提交一次有效动作”视为显式时间推进的主触发点。玩家阅读文本、查看日志和 UI 操作不推进世界。
...
- 第一版默认有效玩家动作都满足 `consumesTick = true`
- 纯 UI 行为不进入世界调度器
...
2. 只更新 backlog：
   ...
   - 到期 tick
   - 摘要标签
3. 每累计 `3` 次玩家有效动作，可挑选极少数远场 NPC 做一次低频补算

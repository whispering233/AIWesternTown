...
第一版不新增 `allowDeepProcessingLlm` 之类授权位；`run_deep_processing` 固定走规则路径。
...
按八阶段把 LLM 权限压缩成最小集。
长动作结算后的 `run_deep_processing` 属于横向编排接口，不在第一版本章节授权范围内，不定义对应 LLM taskKind。
...
第一版不包含 `DeepProcessingPromptInput`（或同义 DTO）。
...
第一版不配置 `deep_processing_integrate`（或同义 taskKind）的预算条目。
...
第一版不定义 deep processing 专用 parser 合并策略；`run_deep_processing` 的结构化输出由规则层直接生成并校验。
...
| 其余 taskKind | 默认关闭 |
| `run_deep_processing` 的独立 LLM 调用 | 关闭（固定规则路径） |

...
当前可玩切片以 `starter-town` 为起点：Web 主界面采用 `Context / Narrative / Map = 1:3:1` 三栏布局，左栏承载玩家状态、日志和人物卡，主栏承载场景叙事、聊天、生成选项和自由输入，右栏承载总地图、当前位置和移动选项。`content/starter-town` 的玩家可见场景、NPC 和 item 文本默认使用中文。
...
首次启动或清理过 `dist/` 后，先准备 workspace 构建产物：
...
- `apps/web`：浏览器游戏主界面、三栏 shell、叙事交互、命令输入、地图与移动入口。
...
- `content/starter-town`：starter town 内容包。玩家可见场景、NPC、item 文案默认中文；ID、tag 和 enum 保持英文稳定。
- `docs`：设计文档、实施方案和任务卡。
- `design/game-ui-system`：游戏 UI 设计系统静态预览，当前以 `design.html` 和 `design.png` 作为主界面实现参考。

## 游戏 UI 与内容包

Web 主界面优先遵循 `docs/design` 与 `design/game-ui-system`：

- 桌面端主布局为 `Context / Narrative / Map = 1:3:1`。
- 移动端降级为纵向堆叠，保留状态、叙事、地图三块职责。
- 左栏 `Context` 负责玩家状态、日志、人物卡和未来 tab 扩展。
- 主栏 `Narrative` 负责地点标签、生成式场景叙事、NPC/玩家聊天、生成选项和自由输入。
- 右栏 `Map` 负责总地图、当前位置和移动选项，不再承载旧版调试页。

内容包边界：

- `content/starter-town/scenes.json` 保存地点名称、摘要、连接关系和场景标签。
- `content/starter-town/npcs.json` 保存 NPC 展示名、职业、公开人格、核心驱动力和短期目标。
- `content/starter-town/items.json` 当前只保存 item 展示名、分类、标签和初始位置；若要增加 item 描述，需要先扩展 `packages/content-schema/src/item.ts`。
- 修改内容包后至少运行 `pnpm --filter @ai-western-town/starter-town-content test`；影响装配流程时继续运行 `pnpm --filter @ai-western-town/app-services test`。
